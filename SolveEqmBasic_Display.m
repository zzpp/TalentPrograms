...
cshow(' ',[Decades GDPYoung' GDPYoung_Model],'%6.0f %8.0f','Year Data Model');

Data =[GDP GDPwkr LFPData EarningsData ConsumpYoungData WageGapData(2:4,:)' EarningsData_g'];
Model=[YModel YwkrModel LFPModel EarningsModel ConsumpYoungModel WageGapModel(2:4,:)' EarningsModel_g'];
DataGrowth=log(Data(1,:)./Data(Nyears,:));
ModelGrowth=log(Model(1,:)./Model(Nyears,:));
GrowthShare=(ModelGrowth./DataGrowth)'*100;
tle={'GDP per person','GDP per worker','Labor Force Participation (LFP)','Earnings','ConsumpYoung (market)','WageGapWW','WageGapBM','WageGapBW','EarningsWM','EarningsWW','EarningsBM','EarningsBW'};
disp ' '; disp ' ';
