...
for loopthome=1:2;  % The first time we use time-varying thome, the second we impose the min across decades
    ...
    for t=1:Nyears;
		...
		wHomegridtable(t,loopthome)=wHome;
        wHome = fminbnd(f, max(wHome-(step-1),1),wHome+(step-1));
		
		%Last check if wHome satisfies mwm>0 and existence of solution for s
        wHome=solveWHomeGRID(wm_tauw,wm_tauh,wHome,wagebar,pt,lambda,beta,eta,theta,gam,FARM,phiFarm(t),pHome,thome,Tbar(:,t));
...
printoutziho(['solveforw_test_grid_fminbnd_' num2str(solveforstage) '.xlsx'],1, wHomeoriginal,numberjtable,minerrtable,wHomegridtable,wHometable);
